...
#[tokio::main]
pub async fn run(validator_vote: String) {
    let keypair = "./id.json".to_string();
    ...
    send_transaction(validator_vote.to_string(), validator_index, &keypair.to_string(), &simulate, &cluster,compute_unit_price.expect("REASON")).await;    
}